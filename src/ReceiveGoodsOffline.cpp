...
void Widget::on_RecvGoodsCommEntBack_clicked()
{
    ...
    ui->RecvGoodsChallanIdLEdit->setText("");
...
{
    if(ui->RecvGoodsTruckChitNoLEdit->text().size() < 5)
    {
        if(LangMode == 1)
    ...
    QString ChallanID = ui->RecvGoodsChallanIdLEdit->text();

    if(ChallanID.size() < 5)
    {
        //P should check here as per dept descision
        if(LangMode == 1)
            miscObj.DisplayWideMsgBox((char *)"Invalid Challan Number");
        else
            miscObj.tel_DisplayWideMsgBox((char *)"अमान्य चालान संख्या !");
        return;
    }
    else
    {
//        qDebug() << "validation Challan no.:" << ChallanID.left(0).toUpper()<<"\t"<< ChallanID.left(1).toUpper();
        if((ChallanID.left(1).toUpper() != "R") && (ChallanID.left(1).toUpper() != "E"))
        {
            if(LangMode == 1)
                miscObj.DisplayWideMsgBox((char *)"Invalid Challan Number");
            else
...
//    sprintf(TmpStr,"select (select count() from OfflineStockReceive where TruckChitNum='%s' COLLATE NOCASE),(select count() from OfflineStockReceive where Challan='%s' COLLATE NOCASE)",ui->RecvGoodsTruckChitNoLEdit->text().toAscii().data(),ui->RecvGoodsChallanIdLEdit->text().toAscii().data());

    sprintf(TmpStr,"select count() from OfflineStockReceive where Challan='%s' COLLATE NOCASE",ui->RecvGoodsChallanIdLEdit->text().toAscii().data());

    qDebug()<<"TmpStr :"<<TmpStr;

    if(!DupQuery.exec(TmpStr))
    {

    }
    DupQuery.next();
    qDebug()<<"DupQuery.value(0).toInt() :"<<DupQuery.value(0).toInt();
//    qDebug()<<"DupQuery.value(1).toInt() :"<<DupQuery.value(1).toInt();

//    if(DupQuery.value(0).toInt()!=0)
//    {
//        miscObj.DisplayWideMsgBox((char *)"Entered Truckchit Number already received");
//        return;
//    }

    if(DupQuery.value(0).toInt()!=0)
    {
        miscObj.DisplayWideMsgBox((char *)"Entered Challan Number already received");
        return;
    }
...
void Widget::PrintRecvGoodsRecpt()
{
    ...
    //if((LanguageSelected == ENGLISHLANG)||(LanguageSelected == HINDILANG))
    {
        ...
        query.clear();
        if(!query.exec("select ShopID,Name from ShopOwnerDetails"))
        {
        }
        query.next();
        memset(TmpStr,0x00,sizeof(TmpStr));
        sprintf(TmpStr,"FPS Owner Name : %s",query.value(1).toString().toAscii().data());
        prn_write_text((unsigned char *)TmpStr,strlen(TmpStr),0);


        memset(TmpStr,0x00,sizeof(TmpStr));
        sprintf(TmpStr,"FPS ID.        : %s",query.value(0).toString().toAscii().data());
        prn_write_text((unsigned char *)TmpStr,strlen(TmpStr),0);
        ...
        query.clear();
        query.prepare("select AllotMonth,AllotYear from KeyRegister limit 1");
        if(!query.exec())
        {
            qDebug()<<query.lastError();
        }
...
void Widget::on_RecvGdsConfirmSubmit_clicked()
{
    ...
    for(int i=0;i<ui->RecvGdsCommDEntryTable->rowCount();i++)
    {
        ...
        if((RecvQtyData->text().toFloat()>0)&&((RecvCommData->currentText().compare("SELECT COMMODITY")!=0)&&((RecvSchemeData->currentText().compare("SELECT SCHEME TYPE")!=0))))
        {
            ...
            memset(TmpStr,0x00,sizeof(TmpStr));
            if(DealerLoginBy == 2)
                sprintf(TmpStr,"insert into OfflineStockReceive values('%s','%s','%s','%s',(select schemeId from schemeMaster where schemeName = '%s'),(select commCode from commodityMaster where commNameEn = '%s'),'%s','%s',(select distinct(AllotMonth) from KeyRegister limit 1),(select distinct(AllotYear) from KeyRegister limit 1),'%02d-%02d-%04d %02d:%02d:%02d','N','Q')",ui->RecvGoodsTruckChitNoLEdit->text().toAscii().data(),ui->RecvGoodsChallanIdLEdit->text().toAscii().data(),ui->RecvGoodsVechNoLEdit->text().toAscii().data(),ui->ldtShpId->text().toAscii().data(),RecvSchemeData->currentText().toAscii().data(),RecvCommData->currentText().toAscii().data(),RecvQtyData->text().toAscii().data(),ui->RecvGdsCommDEntryTable->item(i,3)->text().toAscii().data(),QDateTime::currentDateTime().toString("dd").toInt(),QDateTime::currentDateTime().toString("MM").toInt(),QDateTime::currentDateTime().toString("yyyy").toInt(),QDateTime::currentDateTime().toString("hh").toInt(),QDateTime::currentDateTime().toString("mm").toInt(),QDateTime::currentDateTime().toString("ss").toInt());
            else
                sprintf(TmpStr,"insert into OfflineStockReceive values('%s','%s','%s','%s',(select schemeId from schemeMaster where schemeName = '%s'),(select commCode from commodityMaster where commNameEn = '%s'),'%s','%s',(select distinct(AllotMonth) from KeyRegister limit 1),(select distinct(AllotYear) from KeyRegister limit 1),'%02d-%02d-%04d %02d:%02d:%02d','N','P')",ui->RecvGoodsTruckChitNoLEdit->text().toAscii().data(),ui->RecvGoodsChallanIdLEdit->text().toAscii().data(),ui->RecvGoodsVechNoLEdit->text().toAscii().data(),ui->ldtShpId->text().toAscii().data(),RecvSchemeData->currentText().toAscii().data(),RecvCommData->currentText().toAscii().data(),RecvQtyData->text().toAscii().data(),ui->RecvGdsCommDEntryTable->item(i,3)->text().toAscii().data(),QDateTime::currentDateTime().toString("dd").toInt(),QDateTime::currentDateTime().toString("MM").toInt(),QDateTime::currentDateTime().toString("yyyy").toInt(),QDateTime::currentDateTime().toString("hh").toInt(),QDateTime::currentDateTime().toString("mm").toInt(),QDateTime::currentDateTime().toString("ss").toInt());
            qDebug()<<TmpStr;
            query.clear();
            query.prepare(TmpStr);
            if(!query.exec())
...
void Widget::on_OfflineReceiveGoodsButton_clicked()
{
//    if(DealerLoginBy == 2){
    ...
    if((DealerLoginBy == 2)||(partialOnlineOfflineStatus.toUpper()=="Y")){
        ...
        if(CheckTxnStopDate("RGO")<0) //checking receive goods offline stop date
            return;
        ...
        if(LangMode == 1)
            miscObj.DisplayWideMsgBox((char *)"Offline stock receiving not allowed");
        else
            miscObj.tel_DisplayWideMsgBox((char *)"ऑफ़लाइन स्टॉक प्राप्त करने की अनुमति नहीं है");
        return;
