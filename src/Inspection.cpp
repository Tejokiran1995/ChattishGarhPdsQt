...
void Widget::on_btn_inspector_clicked()
{
    ...
    //-H 'SOAPACTION: \"http://tempuri.org/getInspectorDetails\"'
...
int Widget::showInspCommDetails()
{
    ...
    ui->tableWidget_InspectorVerificationn->setColumnWidth(1,85);
    ui->tableWidget_InspectorVerificationn->setColumnWidth(2,85);
    ui->tableWidget_InspectorVerificationn->setColumnWidth(3,85);
...
//        if(atof(ui->tableWidget_InspectorVerificationn->item(i,2)->text().toAscii().data()) > 0)
                if(atof(ui->tableWidget_InspectorVerificationn->item(i,2)->text().toAscii().data()) > 0||strcmp(ui->tableWidget_InspectorVerificationn->item(i,2)->text().toAscii().data(),"0")==0)
            receiveflag++;
    }
    if(receiveflag == 0){
        char buff[128];
    ...
    ui->ConsentFormcheckBox_7->setChecked(true);
...
void Widget::on_tableWidget_InspectorVerificationn_cellChanged(int row, int column)
{
    ...
    if(ui->stackedWidget->currentIndex() == 44)
    {
        if(column == 2)
        {
            float fCB,fObservdQty, fvariation;
            ...
            if(ui->tableWidget_InspectorVerificationn->item(row,column)->text().isEmpty())
            {
                ui->tableWidget_InspectorVerificationn->item(row,column)->setText("0.00");
                ui->tableWidget_InspectorVerificationn->item(row,column +1)->setText("0.00");
            ...
            if(res_sys != 0)
            {
                ...
                ui->tableWidget_InspectorVerificationn->item(row,2)->setText("0.00");
                ui->tableWidget_InspectorVerificationn->item(row,column +1)->setText("0.00");

                return;
            }
            //if(atof(buf1)==0)
            if(strcmp(buf1,"0.00") == 0)
            {
                ui->tableWidget_InspectorVerificationn->item(row,2)->setText("0.00");
                ui->tableWidget_InspectorVerificationn->item(row,column +1)->setText("0.00");
            ...
            sprintf(cfObservdQty,"%.3f" , fObservdQty);

            fvariation = fCB - fObservdQty;
            sprintf(cVariation,"%.3f" , fvariation);
...
//void Widget::on_btnScanFpInspAuth_clicked()
//{
//    if(!ui->ConsentFormcheckBox_7->isChecked())
//    {
...
FUSION_IMPL:
    ...
    */
    ...
    //-H 'SOAPACTION: \"http://tempuri.org/getAuthenticateNICAuaInspectionRD2\"'
    sprintf(cmdCurl, "curl -X POST -H 'Content-type: text/xml' -d @%s %s > %s -v -m %d", FileName, cUrlName, FileNameResp, responseTimedOutTimeInSec.toInt());
...
int Widget::prepareAuthReqFile_Insp(char *FileName, char *AuthUid, char *b64Templ1, char *b64Templ2)
{
    ...
    */
    ...
    fprintf(fp,"<uidNumber>%s</uidNumber>",asciiBase64Enc);
...
int Widget::Submit_Inspector_Verfication()
{
    ...
    //-H 'SOAPACTION: \"http://tempuri.org/inspPushCBData\"'
    ...
    if(Get_transactionID(cRespfilename) != 0)
    {
        return -3;
    }
}
...
//        sprintf(prn,"%-12s  %-0.2f   %-0.2f   %-0.2f",ui->tableWidget_InspectorVerificationn->item(i,0)->text().toAscii().data(), atof(ui->tableWidget_InspectorVerificationn->item(i,1)->text().toAscii().data()), atof(ui->tableWidget_InspectorVerificationn->item(i,2)->text().toAscii().data()), atof(ui->tableWidget_InspectorVerificationn->item(i,3)->text().toAscii().data()));
        ...
        sprintf(prn,"%7s   %s     %s     %s",ui->tableWidget_InspectorVerificationn->item(i,0)->text().toAscii().data(), ui->tableWidget_InspectorVerificationn->item(i,1)->text().toAscii().data(),ui->tableWidget_InspectorVerificationn->item(i,2)->text().toAscii().data(),ui->tableWidget_InspectorVerificationn->item(i,3)->text().toAscii().data());
    ...
    memset(prn, 0, sizeof(prn));
    sprintf(prn, "      Quantity in Kgs/Ltrs");
    prn_write_text((unsigned char *)prn, strlen(prn), 0);
...
//    str= QString::fromUtf8("खाद्य एवं उपभोक्ता संरक्षण विभाग");
...
//    str.clear();
    ...
    while(i<iRow)
    {
        ...
        */
        ...
        HindiNameComm=ui->tableWidget_InspectorVerificationn->item(i,0)->text();
        str= HindiNameComm+"     "+ui->tableWidget_InspectorVerificationn->item(i,1)->text()+"    "+ ui->tableWidget_InspectorVerificationn->item(i,2)->text()+"      "+ui->tableWidget_InspectorVerificationn->item(i,3)->text();
        prepareBmp(QString::fromUtf8(str.toUtf8()),22,QString::fromUtf8(str.toUtf8()),18,ALIGN_LEFT);
    ...
    QString tHeading  = QString::fromUtf8("    किलोग्राम/लीटर में मात्रा");
