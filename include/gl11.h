...
int SCR_getATR(unsigned char * , int * );
/** @brief
 * This API is used to get ATR from smart card This API is called only after the SCR_powerup API.
 *
 *Returns
 *           ’0’ on success 
 *           ’-1’ on failure
...
int SAM_PowerUp(int,int);
int gl11_gpsctrl(int);
int RFFieldOff(void);
//Added by Madhavi
...
int gl11_eth_control(int);

int SCR_GetSB(unsigned char* Ver,unsigned int *len);

int SCR_GetFV(unsigned char* Firm,unsigned int *rlen);

int gl11_ifd_control(int);

int gl11_internal_camflash(int state);

int gl11_audio_vol_control(int state);
                
//Added by Rohith
int gl11_getuid(char *uid);

int gl11_bluetooth_control(int);


/*int Check_Fuse_fp(void);
int Write_Fuse_fp(void);
int Generate_License_fp(void);
int iimfuse_open();
int iimfuse_blow(int, int, int);

int iimfuse_close();
*/
//int Check_Fuse(void);
//int FP_Fuse_License(void);
//int Generate_License(void);

//void gl11_eth_control(int state);
//int SCR_getATR()
//int SCR_getATR(unsigned char* ,unsigned int *);
#endif
